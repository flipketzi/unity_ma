...
public class MJPEGStreamDecoder : MonoBehaviour
{
    ...
    [SerializeField] bool showDebugMessage = false;
    [SerializeField] string defaultStreamURL = "http://192.168.178.46:8000/stream.mjpg";
    ...
    void ReadMJPEGStreamWorker(int id, string url)
    {
        ...
        while (buffer != null)
        {
            if (threadID != id) return; // We are no longer the active thread! stop doing things dammit!
            if (!buffer.CanRead)
            {
